...
pub struct PgGraphCypherCall {
    pub graph_name: String,
    pub cypher: String,
}

/// Extract the deliberately narrow PostgreSQL graph adapter call:
/// `SELECT * FROM graph.cypher('graph-name', 'MATCH ...')`.
///
/// Returning an error after the `graph` namespace is recognized prevents the
/// normal translator from silently stripping qualification and resolving a
/// different function.
pub fn try_extract_graph_cypher(
    parse_result: &ParseResult,
) -> Result<Option<PgGraphCypherCall>, ParseError> {
    use pg_query::protobuf::node::Node;
    use pg_query::NodeRef;

    let nodes = parse_result.protobuf.nodes();
    let Some(NodeRef::SelectStmt(select)) = nodes.first().map(|node| &node.0) else {
        return Ok(None);
    };
    let Some(from) = select.from_clause.first() else {
        return Ok(None);
    };
    let Some(Node::RangeFunction(range)) = &from.node else {
        return Ok(None);
    };
    let function_node = range
        .functions
        .first()
        .ok_or_else(|| ParseError::ParseError("graph range function is empty".to_owned()))?;
    let function_node = match &function_node.node {
        Some(Node::List(list)) => list.items.first(),
        Some(Node::FuncCall(_)) => Some(function_node),
        _ => None,
    };
    let Some(function_node) = function_node else {
        return Ok(None);
    };
    let Some(Node::FuncCall(function)) = &function_node.node else {
        return Ok(None);
    };
    let name = function
        .funcname
        ...
        .filter_map(|part| match &part.node {
            Some(Node::String(value)) => Some(value.sval.to_ascii_lowercase()),
            _ => None,
        })
        .collect::<Vec<_>>();
    if name.first().is_none_or(|part| part != "graph") {
        return Ok(None);
    }
    if name.as_slice() != ["graph", "cypher"] {
        return Err(ParseError::ParseError(format!(
            "unsupported graph API function {}",
            name.join(".")
        )));
    }
    ...
    {
        return Err(ParseError::ParseError(
            "graph.cypher must be the sole source of SELECT * without SQL filtering, grouping, ordering, or limits"
                .to_owned(),
        ));
    }
    let target_is_star = select.target_list.first().is_some_and(|target| {
        matches!(
            &target.node,
            Some(Node::ResTarget(target))
                if matches!(target.val.as_deref().and_then(|value| value.node.as_ref()), Some(Node::ColumnRef(column)) if column.fields.iter().any(|field| matches!(field.node, Some(Node::AStar(_)))))
        )
    });
    if !target_is_star {
        return Err(ParseError::ParseError(
            "graph.cypher currently requires SELECT *".to_owned(),
        ));
    }
    if function.args.len() != 2 {
        return Err(ParseError::ParseError(
            "graph.cypher requires exactly two positional string arguments: graph name and Cypher text"
                .to_owned(),
        ));
    }
    let graph_name = graph_string_argument(&function.args[0]).ok_or_else(|| {
        ParseError::ParseError(
            "graph.cypher graph name must be a positional string literal; named arguments, regclass/OID values, and compound values are unsupported"
                .to_owned(),
        )
    })?;
    let cypher = graph_string_argument(&function.args[1]).ok_or_else(|| {
        ParseError::ParseError(
            "graph.cypher query must be a positional string literal; named arguments and compound values are unsupported"
                .to_owned(),
        )
    })?;
    Ok(Some(PgGraphCypherCall { graph_name, cypher }))
}

fn graph_string_argument(node: &pg_query::protobuf::Node) -> Option<String> {
    use pg_query::protobuf::{a_const::Val, node::Node};

    match &node.node {
        Some(Node::AConst(value)) => match value.val.as_ref()? {
            Val::Sval(value) => Some(value.sval.clone()),
            _ => None,
        },
        _ => None,
    }
...
mod tests {
    ...
    #[test]
    fn extracts_the_scoped_graph_cypher_adapter_call() {
        let parsed =
            crate::parse("SELECT * FROM graph.cypher('social', 'MATCH (n) RETURN n') AS result")
                .unwrap();
        assert_eq!(
            try_extract_graph_cypher(&parsed).unwrap(),
            Some(PgGraphCypherCall {
                graph_name: "social".to_owned(),
                cypher: "MATCH (n) RETURN n".to_owned(),
            })
        );
    }

    #[test]
    fn graph_adapter_rejects_lossy_or_unsupported_postgres_shapes() {
        for sql in [
            "SELECT * FROM graph.cypher(graph_name => 'social', query => 'MATCH (n) RETURN n')",
            "SELECT * FROM graph.cypher(42, 'MATCH (n) RETURN n')",
            "SELECT * FROM graph.cypher('social', 'MATCH (n) RETURN n') WHERE true",
            "SELECT * FROM graph.shortest_path('social', 1, 2)",
        ] {
            let parsed = crate::parse(sql).unwrap();
            assert!(
                try_extract_graph_cypher(&parsed).is_err(),
                "{sql} must fail precisely at the graph adapter boundary"
            );
        }
    }
