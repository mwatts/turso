...
| Status last updated | 2026-08-10 (remediation progress; see §7.0) |
| Mode | Originally a read-only audit; this file is now also the remediation tracker (status + changesets) |
...
**Scope of this document.** It states what is wrong or incomplete in the graph crates, what must become true after fixes, the contracts of the system as built, numbered requirements that an implementer can test, failure modes, non-goals, PR order, and open questions. As remediations land, §7.0 and the requirement/PR rows below record **Status** and **Changeset** so agents do not re-fix closed work. Live corpus totals remain in REPORT.md, not in this file.
...
Four defect classes **did** produce wrong durable graph state or wrong query answers without a clear Cypher error. **All four are fixed** (PR-A–C, `1a5bf497e` + `d212a504c`); kept here as the original problem statement:

1. ~~**DETACH DELETE leaves type-junction rows.**~~ **Done (R1).** DETACH now deletes type-junction rows for removed relationship identities (`1a5bf497e`).

2. ~~**CSR invalidation ignores spill and membership tables.**~~ **Done (R2).** `derive_generation` includes base tables, spill tables, and type/label junctions (`1a5bf497e`).

3. ~~**Path hop order is not forced in SQL.**~~ **Done (R3).** Path aggregates order by `path_position` (`1a5bf497e`; multi-hop session test).

4. ~~**Write-path DISTINCT and grouping key rows with Debug text.**~~ **Done (R4).** Typed equality keys on mutation DISTINCT / group-by (`1a5bf497e`).

Related write-path defects (also fixed in the same track):

- ~~AVG non-numeric mix~~ **Done (R5)** — `1a5bf497e`
- ~~Integer ORDER BY via f64~~ **Done (R5)** — `1a5bf497e`
- ~~Integer SUM wrapping~~ **Done (R5)** — `1a5bf497e` + overflow test `d212a504c`
- ~~`decode_mutation_rows` `assert_eq!`~~ **Done (R6)** — `1a5bf497e`
...
After the P0 and P1 work in §7 lands, these invariants hold. Status as of the last tracker update:
...
- `derived_generation` today: hash of schema generation + change tokens of base node/relationship tables **and** spill tables + type/label junctions (`1a5bf497e`, R2).
...
**Status: done** — `1a5bf497e` (PR-A). Tests in session/mutation coverage on that commit.
...
**Status: done** — `1a5bf497e` (PR-A). `catalog.rs` table set expanded.
...
**Status: done** — `1a5bf497e` (PR-B). `multi_hop_path_nodes_list_identities_in_hop_order` in session tests.
...
**Status: done** — `1a5bf497e` (PR-C). `equality_key_for_value` on mutation path.
...
**Status: done** — `1a5bf497e` (AVG / ORDER BY / SUM); `d212a504c` (integer SUM overflow unit test on fold path).
...
**Status: done (pre-remediation track)** — property read off join alias `4b36cd16d`; filter push into join SELECT `bb5bfe094`; core drops redundant parentheses for index seek `27f6a9118`. Recorded in PERFORMANCE_BACKLOG item 6.
...
**Status: partial** — `197091d6e` hop-based expand costs, session budgets, LIMIT→max_paths path cap; fuller session-vs-session knob story may still need PR-J follow-up.
...
| --- | --- | --- |
| Statement reuse / prepare cache | Constraints yes (LRU drop-one, R17); mutation helpers no | R16 open; R17 done |
| `table_change_token` | Base + spill + junctions in `derive_generation` (R2) | R2 done `1a5bf497e` |
| UPSERT / ON CONFLICT | Unused on MERGE | R8 |
| Physical UNIQUE indexes | Semantic unique is scan-only | R8, residual R15 unique |
...
| Covering index shapes | Pure `count(*)` improved; filter pushdown R18 | More column-free aggregates |
| Ordered JSON aggregates | Ordered by `path_position` (R3) | R3 done `1a5bf497e` |
...
| --- | --- | --- | --- |
| DETACH without type cleanup | Type junction still lists deleted relationship | **Fixed** `1a5bf497e` | R1 done |
| Spill write without token move | Expand returns old adjacency | **Fixed** `1a5bf497e` | R2 done |
| Unordered `json_group_array` | `nodes(p)` permutes | **Fixed** `1a5bf497e` | R3 done |
| Debug DISTINCT key | Wrong multiset of RETURN rows | **Fixed** `1a5bf497e` | R4 done |
| AVG non-numeric mix | Wrong average | **Fixed** `1a5bf497e` | R5 done |
| Decode arity mismatch | Process panic | **Fixed** soft error `1a5bf497e` | R6 done |
| Concurrent MERGE | Duplicate relationships | Silent possible | R8 open |
| REMOVE label unsupported | Missing feature or vague bind error | Incomplete | R9 open |
...
### 7.0 Remediation status (backfill)

Update this table whenever a PR lands. **Changeset** is a git object on
`feature/graph-frontend` (short SHA from `git log --oneline`). Prefer short
subject lines from that log.

| PR | Requirements | Status | Changeset(s) | Notes |
| --- | --- | --- | --- | --- |
...
**Adjacent work already on branch (not PR-A–J slots, still useful):**

| Topic | Changeset(s) | Maps to |
| --- | --- | --- |
| Source-scoped constraint validation (types, not rows) | `379bdf9e0` | pre-R15; still required |
| Statement cache for constraint/probe SQL | `df549ed80` | PERFORMANCE_BACKLOG item 2; R17 builds on it |
| Generation triggers deleted; tokens for CSR | `53989aa59`, `1c333a51c`, `207105537` | R14 history; invalidation design |
| Filter pushdown + indexable compares | `4b36cd16d`, `bb5bfe094`, `27f6a9118` | R18 |
| Hop-based expand costs, session budgets, LIMIT path cap | `197091d6e` | R20 partial |
| Cypherbench sample+full release numbers | `24fc06518` | evidence, not a requirement id |
...
| Requirement | Minimum new test | Status |
| --- | --- | --- |
| R1 | DETACH then empty type junction for deleted rel | done `1a5bf497e` |
| R2 | Spill-only write moves derived generation / rebuilds expand | done `1a5bf497e` |
| R3 | Multi-hop `nodes(p)` order | done `1a5bf497e` (`multi_hop_path_nodes_list_identities_in_hop_order`) |
| R4 | DISTINCT with values that Debug may collide | done `1a5bf497e` |
| R5 | AVG / big integer ORDER BY / SUM overflow | done `1a5bf497e` + `d212a504c` |
...
Do not combine PR-A with PR-J. Do not land path-order changes without R3’s multi-hop test (already landed with PR-B).
...
Counts at audit time vs after backfill (done = fully closed; partial counted open until residual closed).

| Class | At audit | Still open (incl. partial) | Done |
| --- | ---: | ---: | ---: |
| must (R1–R14) | 14 | **7** (R7–R13) | **7** (R1–R6, R14) |
